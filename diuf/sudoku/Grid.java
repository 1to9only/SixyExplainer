...
public class Grid {
    ...
    private Cage[] cages = new Cage[6];

    private int isSudoku;  // 1=isSudoku (default), 0=isPencilMarks (set when PencilMarks is loaded)
    ...
    public Grid() {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            cages[i] = new Cage(i / 3, i % 3);   // verified
        }
        isSudoku = 1;
    }
    ...
    /**
     * Get the cage at the given index.
     * Cages are numbered from left to right, top to bottom.
     * @param num the index of the cage to get, between 0 and 5, inclusive
     * @return the cage at the given index
     */
    public Cage getCage(int num) {
        return this.cages[num];
    }
    ...
    /**
     * Get the cage at the given location
     * @param vPos the vertical position, between 0 to 2, inclusive
     * @param hPos the horizontal position, between 0 to 2, inclusive
     * @return the cage at the given location
     */
    public Cage getCage(int vPos, int hPos) {
        return this.cages[vPos * 3 + hPos]; // verified
    }
    ...
    /**
     * Get the 3x2 block at the given location
     * @param x the horizontal coordinate
     * @param y the vertical coordinate
     * @return the block at the given coordinates (the coordinates
     * are coordinates of a cell)
    ...
    public Cage getCageAt(int x, int y) {
        return this.cages[(y / 3) * 3 + (x / 2)]; // verified
    }
    ...
    public class Row extends Region {
        ...
        public boolean crosses(Region other) {
            ...
            } else if (other instanceof Cage) {
                Cage cage = (Cage)other;
                return rowNum / 3 == cage.vNum;
    ...
    /**
     * A 3x2 block of a sudoku grid.
     */
    public class Cage extends Region {

        private int vNum, hNum;

        public Cage(int vNum, int hNum) {
            this.vNum = vNum;
            this.hNum = hNum;
        }
        ...
        @Override
        public Cell getCell(int index) {
            return cells[vNum * 3 + index / 2][hNum * 2 + index % 2]; // verified
        }

        @Override
        public int indexOf(Cell cell) {
            return (cell.getY() % 3) * 2 + (cell.getX() % 2); // verified
        }
        ...
        public boolean crosses(Region other) {
            if (other instanceof Row) {
                ...
                Cage cage = (Cage)other;
                return this.vNum == cage.vNum && this.hNum == cage.hNum;
            } else {
                return super.crosses(other);
            }
        }
